  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form @submit.native.prevent>
        ...
        <el-select
          v-if="['table'].includes(re)"
          v-model="listQuery.openStatusList"
          @change="fetchData"
          placeholder="卡片状态"
          style="width: 200px"
          class="filter-item"
        ...
        <el-select
          v-if="['table', 'serv', 'ca', 'lr_pre', 'ro_pre', 'lr', 'ro'].includes(re)"
          v-model="listQuery.idArea"
          @change="fetchData"
          placeholder="开户地区"
          style="width: 120px"
          class="filter-item"
        ...
        <el-select
          v-if="['table', 'cs', 'settle', 'cs_re'].includes(re)"
          v-model="listQuery.authMethod"
          @change="fetchData"
          placeholder="认证方式"
          style="width: 120px"
          class="filter-item"
    ...
    >
      ...
      <el-table-column label="划拨状态" width="120" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="cfg.enum.assign_status.filter(e => { return e.id === scope.row.assign_status })[0].type"
          >{{ cfg.enum.assign_status.filter(e => { return e.id === scope.row.assign_status })[0].value }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="卡片状态" width="120" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="cfg.enum.card_status.filter(e => { return e.id === scope.row.card_status })[0].type"
          >{{ cfg.enum.card_status.filter(e => { return e.id === scope.row.card_status })[0].value }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="实名状态" width="120" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="cfg.enum.real_name_auth.filter(e => { return e.id === scope.row.real_name_auth })[0].type"
          >{{ cfg.enum.real_name_auth.filter(e => { return e.id === scope.row.real_name_auth })[0].value }}</el-tag>
        </template>
      </el-table-column>
